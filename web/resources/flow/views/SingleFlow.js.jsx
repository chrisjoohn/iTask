...
class SingleFlow extends Binder {
  ...
  render() {
    ...
    const { defaultTask, flowStore, match, taskStore, dispatch } = this.props;
    ...
    const completedTaskItems =
      taskListItems && taskListItems.filter((item) => item.complete);
          ...
          <div style={{ opacity: isFlowFetching ? 0.5 : 1 }}>
            ...
            {isTaskListEmpty ? (
              ...
              <div style={{ opacity: isTaskListFetching ? 0.5 : 1 }}>
                <ul>
                  {incompleteTaskItems.map((task, i) => (
                    <li key={task._id + i}>
                      <input
